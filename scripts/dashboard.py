...
def render(user, dark):
    ...
    text(24,30,'02 / GITHUB ACTIVITY',11)
    text(594,30,datetime.datetime.now(datetime.timezone.utc).strftime('UPDATED %d %b %Y / UTC').upper(),10,muted)
    out.append(f'<path d="M24 46H816" stroke="{line}"/>')
    calendar=user['contributionsCollection']['contributionCalendar']
    text(24,86,f"{calendar['totalContributions']:,}",30)
    text(145,84,'contributions / past year',12,muted)
    ...
    text(24,210,'Contribution activity over the past 12 months.',10,muted)
    ...
    text(24,257,'PRIMARY LANGUAGES',11)
    text(24,278,'Public original repos, counted once each.',10,muted)
    ...
    for i,(language,count) in enumerate(counts.most_common()):
        ...
        text(x,y,language,11); text(x+215,y,str(count),11)
...
if __name__ == '__main__':
    ...
    for theme in ('light','dark'):
        (ROOT/'assets'/f'activity-{theme}.svg').write_text(render(user,theme=='dark'))
