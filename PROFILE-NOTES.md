...
The README is a sequence of linked display panels. Roboto glyphs are outlined in SVG so GitHub cannot substitute a font. Roboto is bundled under the SIL Open Font License in assets/fonts/OFL.txt. Install scripts/requirements.txt before running the generators. Run scripts/panels.py to rebuild static panels, scripts/header.cjs for the PNG header, and scripts/dashboard.py for live metrics. The activity panel links to GitHub’s native activity overview. Interactive JavaScript cannot run within the README.
